...
residualPlots(zoop_lm, cex.lab=1.4, plot=FALSE)[3,]
```
</div>
...
## F-Test
```{r zanova}
knitr::kable(broom::tidy(anova(zoop_lm)))
```
...
```{r visreg}
par(mfrow=c(1,2))
visreg::visreg(zoop_lm, type="effect")
par(mfrow=c(1,1))
```

## Comparison of Differences at Average of Other Treatment
```{r lsmeans}
contrast( lsmeans(zoop_lm, "treatment"), "tukey", adjust="tukey")
```
