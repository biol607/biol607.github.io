...
## 
<Br><br><br>
<h1>https://etherpad.wikimedia.org/p/607-t_tests</h1>
...
<span class="fragment">$$\Large z = \frac{\bar{X} - \mu}{\sigma_{\bar{X}}}$$</span>

## This is a Golem

<div id="left"><br>$$\LARGE z = \frac{\bar{X} - \mu}{\sigma_{\bar{X}}}$$</div>
...
```{r t-anim, cache=TRUE,  fig.show = "animate", cache=TRUE}
library(gganimate)
library(dplyr)
library(tidyr)
set.seed(31415)

samps <- data.frame(sample_size=3:20) %>%
  group_by(sample_size) %>%
  do(samp = rnorm(.$sample_size[1])) %>%
  unnest(samp)
...
gg_animate(dists, interval=0.6, loop=1)
```
...
```{r t_test}
t.test(`Squamosal horn length` ~ Status, data=lizards)
```
