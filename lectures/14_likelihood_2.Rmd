...
<center>Variation in Predator Behavior</center>
...
library(ggplot2)
ggplot() +
  aes(x=predict(puffer_mle), y=residuals(puffer_mle)) +
  geom_point() + 
...
qqnorm(residuals(puffer_mle), cex.lab=1.5)
qqline(residuals(puffer_mle))
```
</div>

## But - What do the Likelihood Profiles Look Like?
```{r profile}
plot(profile(puffer_mle), cex.lab=1.5)
```
...
<Br>
```{r mle_chisq}
anova(puffer_mle, puffer_null_mle)
```